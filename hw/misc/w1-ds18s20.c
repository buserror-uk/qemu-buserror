 ...
 * well it'll behave sufficiently that linux's w1-gpio driver will
 * be fooled into talking to it.
 * w1-gpio (and other onewire masters?) attemps to fudge the bit timing
 * to try to adapt to 'bad' wires, most real hardware sensors must have a
 * PLL of sort are seems to be able to adapt, this implemention doesn't
 * and 'sometime' drops from the bus for a short time, in a way it's
 * rather nice as it simulates a moderatly bad wire.
 *
